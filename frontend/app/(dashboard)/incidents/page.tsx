  ...
  const triggerPagerDutyAlert = async (serviceId?: string) => {
    try {
      ...
      if (response.status === 'success' && response.data.success) {
        ...
        console.warn('⚠ Failed to trigger PagerDuty alert:', response.data?.message || 'Unknown error');
    ...
    try {
      ...
      if (isAllServices) {
        ...
        setChaosResults(prev => [
          ...
          ⚠ Partial success: ${failureCount} services failed to nuke,
          '',
          '📋 Detailed Results:',
          ...allResults
        ]);
        ...
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-red-600">
              ...
              ⚠ {selectedService ? 'SERVICE' : 'INFRASTRUCTURE'} CHAOS WARNING ⚠
            </DialogTitle>
              ...
              <Alert className="border-yellow-200 bg-yellow-50">
                ...
                <AlertDescription className="text-yellow-700">
                  {Object.keys(serviceStatuses).length > 1 ? (
                    ...
                    'Infrastructure has been successfully compromised. Check your monitoring systems and PagerDuty for alerts. Your KubeDoo agent should start analyzing and attempting to resolve these incidents automatically.'
                  )}
                </AlertDescription>
              </Alert>
            )}
